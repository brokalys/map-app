...
import type { VZDLand } from 'src/types/vzd';
...
type DataFormat = Property | VZDLand;
...
function getCellTextAlign(cell: UseTableCellProps<DataFormat>) {
  if (['type'].includes(cell.column.id)) {
    return 'left';
  }
...
function useQuerystringFilters() {
  const [{ source, type }] = useBuildingFilters();
...
interface LandTableProps {
  estate: (Land & { data: DataFormat[] }) | undefined;
}

const LandTable: React.FC<LandTableProps> = (props) => {
